...
function buildElementProperties(models, modelName,
  ...
  for (let i in elements) {
    ...
    if (element.type) {
      ...
      propertyData = createPropertyData(checkAndConvertToNumber(type), element);
      propertyList[element.qname.name] = propertyData;
    }
...
function createPropertyData(typeName, element) {
  return (element.isMany && element.isMany == true) ?
